...
public class GrainDaoImpl extends GenericHibernateDao<Grain> implements IGrainDao{
	...
	public List findFriendsGrain(String userId, String mcateId,
		...
		String sql = "SELECT DISTINCT g.gid AS grainId, f.user_id AS userId, g.lon AS lon, g.lat AS lat, u.portrait_small AS userSmallPortait "+
						   "FROM grain AS g, 	`user` AS u, friend AS f " + 
						   "WHERE " +
						   "u.user_id = ? AND " +
						   "f.user_fid = u.user_id AND " +
						   "f.user_id = g.user_id AND " +
						   "f.flag = 1 AND " +
						   "g.mcate_id = ?";
		
		Session session = getSession();
		List result = session.createSQLQuery(sql).setParameter(0, userId).setParameter(1, mcateId).list();
		session.close();
		return result;
	}
