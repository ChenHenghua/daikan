...
public class GrainServiceImpl implements IGrainService{
	...
	public List findFriendsGrain(String userId, String mcateId,
		...
		List<Object[]> list = grainDao.findFriendsGrain(userId, mcateId, cityCode, lon, lat, radius);
		List grains = new ArrayList();
		for(Object[] arr: list){
			HashMap map = new HashMap();
			map.put("grainId",arr[0]);
			map.put("userId",arr[1]);
			map.put("lon",arr[2]);
			map.put("lat",arr[3]);
			map.put("userSmallPortait",arr[4]);
			grains.add(map);
		}
		return grains;
	}
